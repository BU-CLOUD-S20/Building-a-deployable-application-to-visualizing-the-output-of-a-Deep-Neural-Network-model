...
model by storing test images in a file system and examined for correctness through the user interface. This user interface should be deployed to the cloud.
...
* UI/Frontend
    ...
    * Allows images to be shown with either hard coded or API-returned rectangle coordinates for object classification and tracing (maybe remove "hard-coded" since that is just a stepping-stone to help with development but not part of the final solution. Same applies to some of the bullet points below.)
...
* REST API
    ...
    * API scenarios include Classification, Similarity, Detection and possibly Keypoint, but can support user defined scenarios as well
* Image Similarity
 ...
 * Azure Container/Kubernetes: DNN model publishing locations, with focus on simplicity and CPU compute only (GPU's are expensive and would use up the free Azure credits too quickly)
 * REST API: communication end-point where UI-DNN and Azure-published models interact
 * Local Web Server: used to serve/execute UI-DNN-Sim in all instances (the goal should be to also host the image similarity UI on the cloud. I checked with two software engineers here and they said that this could also be done with javascript, and hence re-use all the work done from the othe UIs); used to serve/execute UI-DNN in MVP
 * Azure Blob Storage: alternate publishing location for UI-DNN (as stretch goal)  Not sure what this means. What is the other "alternate publishing location for UI-DNN". Ps. seems odd to have definition what is stretch, what is mvp, etc here in this paragraph. Could instead only mentioned it under either "scope and features" or "acceptance criteria" to more clearly point out what the mvp is.
...
Great figure! The UI-DNN-Sim also would be calling the Rest API, not for the reference images (those would be precomputed offline and locally) but for a query image when it is uploaded via the UI.
...
Many design decisions have not been made yet. Three notable decisions evident above:
  ...
  * UI-DNN-Sim as separate code: since it will return a list of similar images to be displayed (vs. text / bounding boxes for the other models), the interface will differ significantly; also, since it will only interface with local models and code, there is no requirement to publish this element to Azure Blob Storage. Once a query image is uploaded via the UI, it would call a (slightly modified ) Rest API to get its feature embeddings and hence could be uploaded. This is a bit of a stretch goal but seems doable to me.
...
Minimum acceptance criteria is a deployed UI (UI-DNN) which calls a cloud-based REST API and visualizes its output for multiple DNN scenarios; except for UI of “similarity” DNN models (UI-DNN-Sim) which will also call a could-based REST API for each uploaded query image.

Stretch goals are:
  * Deployment of the UI-DNN-Sim site to the cloud.
  * Use Azure blob storage as an alternate publishing location for UI-DNN. Don't understand.
...
## 6.Release Planning:
Plans and meeting notes are on the Trello board: https://trello.com/b/Cz7rU2Kz  Is this link important for me to look at? If so I'd need the required permission.

Which week is today (Feb 3rd, is it the 2nd week?)

How "strict" are these release notes, are these set in stone or can these change over time depending on progress?
